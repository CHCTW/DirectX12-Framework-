...
void initializeRender()
{
	...
	rootsig.mParameters[5].mResCounts = 3;
	rootsig.mParameters[5].mBindSlot = 2;
	rootsig.mParameters[5].mConstantData = &scales;



	rootsig.initialize(render.mDevice);

	shaderset.shaders[VS].load("Shaders/TriplanarProjection.hlsl", "VSMain", VS);
	shaderset.shaders[PS].load("Shaders/TriplanarProjection.hlsl", "PSMain", PS);
	shaderset.shaders[HS].load("Shaders/TriplanarProjection.hlsl", "HSMain", HS);
	shaderset.shaders[DS].load("Shaders/TriplanarProjection.hlsl", "DSMain", DS);
...
void loadAsset()
{
	...
	scales.heightscale = 1.0f;
	scales.repeatscale = 0.05f;
	scales.sharpness = 2.0f;
	...
	blockbase.CreateTexture(render, srvheap, DXGI_FORMAT_R8G8B8A8_UNORM, blockbasecolor.mWidth, blockbasecolor.mHeight);
	...
	patch.generatePatch(200, 200, 100,100, Triangle, PerlinGenerate, 0, 0.01, 0.01, 50, 1.0, 1.0);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_LEFT)
		scales.heightscale -= 0.01;
	if (key == GLFW_KEY_RIGHT)
		scales.heightscale += 0.01;

	if (key == GLFW_KEY_UP)
		scales.repeatscale += 0.01;
	if (key == GLFW_KEY_DOWN)
		scales.repeatscale = max(scales.repeatscale-0.01f,0.0001f);


	if (key == GLFW_KEY_P)
		scales.sharpness += 0.1;
	if (key == GLFW_KEY_O)
		scales.sharpness = max(scales.sharpness - 0.1f, 1.0f);
...
{
	int testnumber = 3;
	windows.initialize(1600, 900, "TriplanarProjection");
	windows.openWindow();
