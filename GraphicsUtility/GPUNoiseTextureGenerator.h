...
struct WorleyFBMNoiseConstData
{
	float scale;
	float time;
	float offsetx;
...
struct PerlinWorleyNoise2DConstData
{
	int octaves;
	float persistance;
	float frequency;
...
public:
	...
	void recordPureGenerateWorleyFBMNoise2D(CommandList &recordlist, Texture& texture, bool reverse = false, float scale = 1.0f, float time = 0.0f,int octave = 5, unsigned int offsetx = 0, unsigned int offsety = 0, int miplevel = 0);
	...
	void recordPureGeneratePerlinWorleyNoise2D(CommandList &recordlist, Texture& texture, float frequency, int octave = 1, float persistance = 1.0f, float time = 0.0f, float valuescale = 1.0f, int repeat = 0, float offsetx = 0, float offsety = 0, int miplevel = 0);
